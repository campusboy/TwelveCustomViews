...
public class CustomViewGroupGridLayout extends ViewGroup {
	...
	public CustomViewGroupGridLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			columns = a.getInteger(R.styleable.CustomViewGroupGridLayout_numColumns, 2);
			margin = (int) a.getInteger(R.styleable.CustomViewGroupGridLayout_itemMargin, 2);
		}
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		int rows = count % columns == 0 ? count / columns : count / columns + 1;// 行数
		if (count == 0)
			return;
